...
# Add $DATADIR/runXXXXX_*.mid to list below, where XXXXX is the run number
 for f in $DATADIR/run44447_*.mid $DATADIR/run44440_*.mid $DATADIR/run44439_*.mid $DATADIR/run44438_*.mid 
...
if [ ! -f $AFILE ];  
	...
	if [ -f $AFILE ];
		then
			SortCodes/SortData $ANALDIR/analysis$h.root $CALFILE Histograms.root 82Sr 196Pt
  	  mv Histograms.root HistFiles/Histograms_82Sr_Pt_$h.root
	fi 

done

gadd -f Sr82_Pt_Summed.root HistFiles/Histograms_82Sr_Pt_*.root
