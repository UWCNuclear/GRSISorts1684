...
# Add $DATADIR/runXXXXX_*.mid to list below, where XXXXX is the run number
 for f in $DATADIR/run44448_*.mid 
...
if [ ! -f $AFILE ];  
	...
	if [ -f $AFILE ];
  	  ...
  	  mv Histograms.root HistFiles/Histograms_NoLasers_82Sr_Pt_$h.root
	fi 

done

gadd -f Sr82_Pt_NoLasers_Summed.root HistFiles/Histograms_NoLasers_82Sr_Pt_*.root
